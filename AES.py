...
# Key Generation functions
    ...
    def key_expansion(self):
        ...
        #number of words 
        Nk = len(self.key) // 4 # Number of words in a key (since each word contains 4 bytes we divide to 4)
        ...
        for i in range(Nk, Nb * (Nr + 1)): # Total number of words = Nb * (Nr + 1)
            ...
            temp = words[i - 1]
            if i % Nk == 0:
                temp = bytes(self.xor_words(self.sub_word(self.rot_word(temp)), self.rcon[i // Nk - 1]))
            
            # append the word to the list of the words
            words.append(bytes(self.xor_words(words[i - Nk], temp)))
        
        # form keys from words, each key contains 4 consequent words
        round_keys = [words[i:i+4] for i in range(0, len(words), 4)]

        return round_keys

cypher = AES(128)
print(len(cypher.round_keys))
for i in cypher.round_keys:
    print(f'len: {len(i)}  key: {[j.hex() for j in i]}')