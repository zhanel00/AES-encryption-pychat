...
def sha256(message):
    ...
    # Pre-processing:
    original_message_length = len(message) * 8
    message += b'\x80'
    message += b'\x00' * ((56 - len(message) % 64) % 64)
    message += original_message_length.to_bytes(8, byteorder='big')

    for chunk_start in range(0, len(message), 64):
        w = []
        for i in range(16):
            w.append(int.from_bytes(message[chunk_start + i*4 : chunk_start + (i+1)*4], byteorder='big'))
    ...
    return (h0.to_bytes(4, byteorder='big') +
